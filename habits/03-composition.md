...
### 렌더 프롭은 "조각을 꽂는 슬롯"일 때만 — 상태 릴레이 채널로 쓰지 마라

위 목록의 렌더 프롭은 **컴포넌트가 화면 조각 하나를 받는 용도**다(`right`, `title`, `headerActions`). 반대로 **상태를 아래로 흘리는 통로**로 렌더 프롭을 쓰면 그건 prop 드릴링을 콜백으로 포장한 것일 뿐이다 — 필드가 늘 때마다 인자가 붙고, 중간 컴포넌트가 전부 그 인자 목록을 알게 돼 확장성이 죽는다.

```tsx
// ❌ 상태 릴레이 채널 — 값·세터를 children 인자로 흘림. 필드가 늘면 인자가 계속 붙는다
<OrderFormShell>
  {({ name, setName, qty, setQty }) => (<><NameField value={name} onChange={setName} />…</>)}
</OrderFormShell>
// ✅ 슬롯 — 넘기는 건 화면 조각뿐. 값은 각 필드가 자기 소유자(폼 컨텍스트)에서 조달
<OrderFormShell title="주문 수정" footer={<SubmitButton />}>
  <NameField />
</OrderFormShell>
```

- 판별 질문: **렌더 프롭의 인자에 상태·세터가 나오는가.** 나오면 채널이니, 그리는 방법을 바꾸는 대신 **상태의 주인**을 찾는다(폼 라이브러리·URL·서버 캐시 — 아래 "얕은 포장" 절의 상한선, [02-structure-cohesion](02-structure-cohesion.md) 변경의 소스).
- 조각의 **개수를 타입으로 닫아야** 하면 슬롯이 아니라 필수 prop 이 맞다(아래 ②-b).

## 추상화는 실제 로직이 정당화할 때만 (얕은 포장 금지)

위 "조합/분리" 원칙의 **브레이크**다. "나눠라"를 곧이곧대로 받으면 **하는 일이 없는 껍데기**(래퍼 훅·래퍼 컴포넌트·context)를 만들게 된다 — 쪼개라 ≠ 빈 간접 레이어를 만들어라.
...
- 얕은 트리(깊이 1~2단)의 폼·모달에서 state를 공유해야 해도 → **`useState` + 명시적 prop이 기본.** context·래퍼 훅·payload 묶음 객체로 성급히 감싸지 말 것. (아래 "중간 핸들러 금지"의 예외다 — 깊이 1~2단에선 부모가 폼 state 를 들고 prop 으로 내리는 건 조율이 아니라 그냥 소유다.)
- **단 여기엔 상한선이 있다.** 편집 대상이 여러 섹션으로 갈리고 필드가 늘어나는 폼이면 `useState` + prop 릴레이가 곧 한계에 온다 — 섹션마다 `value/onChange` 쌍을 내려주고, 부분 저장·dirty 판정을 손으로 굴리게 된다. 그 지점에서는 **폼 라이브러리에 상태 소유권을 넘기고**(예: RHF `FormProvider`) 하위 섹션이 폼 컨텍스트에서 **각자 조달**한다. prop 으로는 읽기 전용 식별자만 내린다.
  - 이건 얕은 포장이 아니다 — **자작 context 로 한 겹 감싸는 것**과 달리 상태의 주인이 실제로 바뀌고, 검증·dirty 추적이라는 로직이 그쪽으로 넘어간다.
  - 그 중간에 **자작 context·래퍼 훅을 끼우는 게 제일 나쁘다.**
- **자기 로직(fetch·파생 계산·검증)을 소유할 때만** 훅·컴포넌트 분리가 정당하다. 판단 질문: **"이 레이어가 로직을 옮기나, 이름만 바꾸나?"** — 이름만 바꾸면 부모에 인라인으로 되돌린다.
  - **단 반복되면 접는다.** 케이스 B 는 **한 번 쓰이는** 통과용이라 지우는 것이다. 같은 레이아웃·라벨 마크업이 N 번 반복되면 로직 0이어도 슬롯 컴포넌트로 접는다 — 그때 지워지는 건 간접 레이어가 아니라 **중복**이고, 인라인 N 벌은 "JSX=화면"을 오히려 묻는다.
- **한 줄짜리에는 이 질문을 그대로 대면 안 된다.** 한 줄이어도 그게 **도메인 정의**면 추출이 맞다. 갈림은 길이가 아니라 성격이다:
...
소비처가 prop을 내려주고 *동시에* 그 값에 맞는 요소를 자기 손으로 만들어야 하면, 계약이 열려 있어 한 몫만 틀려도 조용히 깨진다. → **컴파운드 슬롯이 그 연결을 내부에서 흡수**(내부에서 id 생성해 context로 연결)해 계약을 닫는다.
...
컴파운드는 *내부 연결*(id 맞추기·이벤트·스타일)은 흡수해 닫지만, **개수(cardinality)는 못 닫는다** — children 슬롯은 본질적으로 위치 기반·개수 자유라 소비처가 `.Header`를 **0개(빠뜨림)·2개(중복)** 로 써도 컴파일러가 못 잡는다. 그 조각이 그 컴포넌트의 **핵심 기능**이면 침묵의 고장이 된다. 개수를 런타임에 세려면 렌더 후 DOM을 뒤지는 effect가 필요한데(→ [01-component-design](01-component-design.md) effect 최후 원칙 위반), 그건 겉만 막는 땜빵이다.
...
## 변경 전파 최소화 (blast radius) — 자기 데이터는 자기가 조달, 부모는 구조만

- 컴포넌트(특히 모달)는 **자기 데이터를 내부에서 조달** — 서버 데이터도 prop 주입 대신 직접 `useSuspenseQuery`. 바깥에서 여러 정보를 조합해 넘기면 열림/닫힘 **생명주기에 외부 요인이 끼어 사이드이펙트가 폭발**한다.
- **중간 핸들러 금지** — 부모가 형제 조율용 state·콜백을 소유하지 말 것. 부모는 "무엇을 보여줄지(step/구조)"만, 로직(API 호출·fetch)은 **실제 쓰는 자식이 직접** 소유한다.
