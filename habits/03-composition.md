...
- **단 여기엔 상한선이 있다.** 편집 대상이 여러 섹션으로 갈리고 필드가 늘어나는 폼이면 `useState` + prop 릴레이가 곧 한계에 온다 — 섹션마다 `value/onChange` 쌍을 내려주고, 부분 저장·dirty 판정을 손으로 굴리게 된다. 그 지점에서는 **폼 라이브러리에 상태 소유권을 넘기고**(예: RHF `FormProvider`) 하위 섹션이 폼 컨텍스트에서 **각자 조달**한다. prop 으로는 읽기 전용 식별자만 내린다.
  ...
  - 경계: **필드 몇 개짜리 단일 폼 → `useState` 인라인. 섹션이 갈리는 편집 폼 → 폼 라이브러리.** 그 중간에 자작 context·래퍼 훅을 끼우는 게 제일 나쁘다.
...
컴파운드는 *내부 연결*(id 맞추기·이벤트·스타일)은 흡수해 닫지만, **개수(cardinality)는 못 닫는다** — children 슬롯은 본질적으로 위치 기반·개수 자유라 소비처가 `.Header`를 **0개(빠뜨림)·2개(중복)** 로 써도 컴파일러가 못 잡는다. 그 조각이 그 컴포넌트의 **핵심 기능**이면(예: 드래그 핸들 겸 `aria-labelledby` 대상인 헤더 — 없으면 드래그가 조용히 죽고, 둘이면 id 중복) 침묵의 고장이 된다. 개수를 런타임에 세려면 렌더 후 DOM을 뒤지는 effect가 필요한데(→ [01-component-design](01-component-design.md) effect 최후 원칙 위반), 그건 겉만 막는 땜빵이다.
