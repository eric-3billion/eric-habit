...
> 한 줄 요약: **겉으로 보이는 문제(콜백 주입·동기화)를 고치려 메커니즘을 갈아끼우지 말고, "변경의 소스가 어디 있나"를 의심하라.**
...
1. **보이는 문제가 아니라 소스를 의심하라.** `onXChange` 콜백을 위로 주입하거나, 같은 정보를 여러 state로 복제해 동기화하고 있으면 → 변경의 소스가 아래에 갇힌 신호.
