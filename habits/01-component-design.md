...
- 주석: **코드로 안 보이는 제약**(호출 맥락 가정, 의도적 복제, 연동 지점)**만** — 이건 길어도 남긴다([canonical-examples](canonical-examples.md) `useProjectId`). 아키텍처 내레이션·소유권 주석 금지. 설명 주석이 길어지는 건 코드가 안 읽힌다는 신호다([00](00-intent.md) 증상표).
