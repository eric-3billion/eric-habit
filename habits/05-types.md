...
- **lint 억제 주석으로 사유를 대는 회피는 그 위생 위반보다 더 나쁘다.** `biome-ignore`/`eslint-disable` 로 `!` 를 통과시키려는데 사유가 그럴듯하다면(예: "Suspense 경계 안이라 항상 존재"), 그건 **타입이 그 사실을 표현하지 못한다**는 뜻이다 — 억제하지 말고 값이 항상 존재하도록 **조달 구조를 고친다.**
  ...
  const { data: order } = useSuspenseQuery(orderQueries.detail(id));   // 경계 안: 항상 존재 ([01](01-component-design.md) §3)
  const total = order.total;
  ```
...
4. (금지) **같은 스키마를 두 벌 구현** — 드리프트가 가장 크다. 단 폼·훅·스키마가 실제로 갈리면 그건 복제가 아니라 **분기별 분리**([01 §2](01-component-design.md))이고 억지 통합이 더 나쁘다 — 공유 리프는 `V2 extends V1` 로 드리프트를 누른다. 판별: **두 벌이 같은 것을 다루는가.**
