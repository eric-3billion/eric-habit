...
## 의도가 안 드러날 때 나타나는 신호 4종

겉모습은 달라도 원인은 하나다: **코드가 의도를 드러내지 못해 다른 수단이 그 일을 대신하고 있는 것.** 고칠 것은 겉모습이 아니라 구조다.
...
- 중첩·형제 레이어를 **동의어**로 가르면 접미사가 아무 정보도 주지 못한다 → 각 조각의 **실제 역할**(가드·셸 / 경계·몸통)을 이름에 담는다.
...
**그릇을 주장하는 이름**(`*Config`·`*Context`·`*Options`)은 내용이 하나로 좁혀진 순간 **그 하나를 이름에 박는다.** 이름이 이미 없어진 내용을 계속 내세우면 소비처는 그게 있는 줄 알고 찾는다.
...
- 그런 산술이 필요해졌다는 건 이름이 부족한 게 아니라 **표현이 틀렸다는 신호**다 → 잘 명명하는 대신 **센티넬이 필요 없는 표현으로 바꾼다.**
