---
name: eric-review
description: Eric 스타일 PR 코드리뷰 하네스. 코드 습관 SSOT(../../habits)를 기반으로 6개 렌즈(의도/컴포넌트/함수형·도메인/타입/정확성/테스트)로 PR 변경분을 직접 훑고, 스냅샷 코드로 적대적으로 오탐(false positive)을 자기검증한 뒤, 고가치 발견만 한국어 인라인 코멘트 후보로 뽑아 근거·수정안까지 전건 상세 설명하고, 자동 게시 대신 사용자가 고른 것만 PR에 단다. "리뷰해줘", "내 스타일로 봐줘", "PR 까봐", "code review", PR URL/번호가 주어진 코드리뷰 요청에서 사용.
---
...
핵심 흐름: **메인 루프가 직접 멀티렌즈로 훑고(룰은 habits에서) → 스냅샷 코드로 적대적 자기검증(오탐 컷) → 고가치만 코멘트 후보로 → 후보 전건을 근거·수정안까지 풀어서 설명 → 사용자가 선택 UI로 고른 것만 게시.**
...
7. **코멘트 후보 추리기** — 6번 발견 중 **게시 가치 있는 것만**(이론적 nit/노이즈 제외). 각 후보를 바로 붙일 수 있게 초안:
   - `path`, `line`(side `RIGHT`; 수정 파일은 diff 헌크 안의 라인만), 한국어 `body`(근거 코드 + 수정안 + 정전 매핑 한 줄(해당 시, 8번 룰)).
...
8. **후보 전건 상세 설명** (선택 UI 앞에 반드시 온다):
   ...
   - 후보마다 한 섹션씩, **번호·`파일:라인` 링크·심각도·렌즈**를 달고 본문에 직접 풀어쓴다. 각 섹션에 담을 것:
     ...
     - **수정안** — 붙여넣을 수 있는 코드. 타입/테스트가 잡아주게 되는지도 명시.
     - **정전 매핑(해당할 때만)** — 이 문제가 정전 문헌에 *이름 붙은* 문제면 마지막에 한 줄로 연결: "같은 문제를 Refactoring 은 shotgun surgery 라 부른다" 식. **한 문제에 한 용어** — 다른 문헌의 다른 개념을 별칭처럼 나열하지 않는다. 범위: Clean Code · Clean Architecture · DDD(Evans) · Refactoring(Fowler) · A Philosophy of Software Design(Ousterhout).
       ...
       - 근거 대체 금지: 실패 시나리오·수정안이 본체고 인용은 부록 한 줄. "책에서 그러니까"가 이유가 되면 안 된다(위 "추상적 원칙 나열 금지"의 뒷문 아님).
