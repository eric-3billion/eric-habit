...
2. **head 파일 수집** (워크플로 스크립트는 FS/쉘 접근이 없으니 메인 루프가 준비):
   ```bash
...
4. **워크플로 실행**:
   ...
   Workflow({ scriptPath: "<이 스킬 폴더>/workflow.js",
              args: { baseDir: "<DEST>", diffPath: "<...>/pr.diff",
                      habitsDir: "~/code-style/habits", pr: PR } })
   ```
   각 렌즈 finder가 먼저 자기 habit 파일을 Read → 그 룰로 PR 파일 탐색 → 발견별 적대적 검증(불확실하면 기각) → 확정/조정만 반환.
