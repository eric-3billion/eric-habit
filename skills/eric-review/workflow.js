export const meta = {
  ...
  phases: [
    { title: 'Review', detail: '각 렌즈가 habit 파일을 읽고 PR 파일을 병렬 탐색' },
    { title: 'Verify', detail: '파일을 직접 읽는 회의적 검증자가 각 발견을 confirm/reject (불확실하면 기각)' },
    { title: 'Synthesize', detail: '확정/조정만 dedup + 심각도·렌즈로 정리' }
  ]
}

// 메인 루프가 args로 전달: { baseDir, diffPath, habitsDir, pr }
...
PR 파일은 ${BASE}/ 아래(repo-relative 경로로 보고). 변경 diff: ${DIFF}.

집중: ${lens.focus}

규칙: 파일을 실제로 열어 라인 번호를 확인(추측 금지). 발견마다 정확한 file+line + 무엇이 왜 문제인지(코드 인용 + 어느 habit 룰 위반) + 구체적 처방. 해당 렌즈에 진짜 해당하는 것만, 과잉보고 금지.`,
      { label: `find:${lens.key}`, phase: 'Review', schema: FINDINGS_SCHEMA, agentType: 'Explore' }
    ).then(r => ({ lens: lens.key, findings: (r && r.findings) ? r.findings : [] }))
  },
  found => parallel(found.findings.map((f, i) => () =>
    agent(
`적대적 검증자. 아래 발견을 파일을 직접 열어 confirm 또는 REFUTE하라. 코드로 확인 못 하면 기본값 "rejected".

${ENGINEERING_NOTE}

파일: ${BASE}/${f.file} (라인 ${f.line} 주변을 읽어라)
발견[${found.lens}]: ${f.title}
- severity(주장): ${f.severity}
- detail: ${f.detail}
- suggestion: ${f.suggestion}

스타일 발견이면 habit 룰 기준으로 진짜 지적할 가치가 있는지(주관적 노이즈 아닌지) 판단. 필요하면 ${HABITS}/ 의 해당 룰 파일을 읽어 대조. 정확성 발견이면 실패 시나리오를 구성해보고 불가능하면 기각. 라인 번호 틀렸으면 교정, 심각도 과/소 평가면 조정. 본 코드를 인용하고 판정하라.`,
      { label: `verify:${found.lens}#${i}`, phase: 'Verify', schema: VERDICT_SCHEMA, agentType: 'Explore' }
